...
module RailsWarp
  module JbuilderExtension
    ...
    def warp_ok(**options)
      ...
      # 构建响应哈希
      response_hash = build_response(true, data, msg, code, **options.except(:data, :msg, :code))
    ...
    def warp_fail(**options)
      ...
      data = options[:data]
      # 构建响应哈希
      response_hash = build_response(false, data, msg, code, **options.except(:data, :msg, :code))
      # 将响应哈希的内容合并到当前 jbuilder 对象
      merge!(response_hash)
    end
